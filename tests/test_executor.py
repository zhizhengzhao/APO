...
def test_continue_then_done_runs_two_cycles():
    ...
    V7-γ: agents must now wrap replies in ACTION: submit (no more implicit
    final-reply convention)."""
...
def test_safety_cap_uses_heuristic_fallback():
    ...
    V7-γ: agents must wrap content in ACTION: submit. Heuristic extractor
    searches AgentMessage.text (= submit's ARGS) for role markers."""
...
def test_react_inner_loop():
    ...
    V7-γ: the final step is a plain-text reply with no ACTION block — taken
    as implicit submit. (The legacy ACTION: submit wrapper still works but
    is no longer the advertised protocol.)"""
    ...
    assert turn.n_real_tool_calls == 1
    assert turn.submit_implicit
    assert "5" in turn.text
    assert not turn.hit_cap


def test_parse_action_tools_and_termination():
    """parse_action returns (tool, args) for real tools and (submit/skip, args)
    for termination primitives; None when no ACTION block."""
    assert parse_action("ACTION: python_exec\nARGS: 1+1") == ("python_exec", "1+1")
    assert parse_action("no tool here") is None
    assert parse_action("ACTION: submit\nARGS: hi") == ("submit", "hi")
    assert parse_action("ACTION: skip") == ("skip", "")
    multi = (
        "THOUGHT: think\n"
        "ACTION: web_search\n"
        "ARGS: GRPO algorithm 2025\n"
        "more notes"
    )
    name, args = parse_action(multi)
