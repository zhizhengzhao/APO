...
def fam_researcher_verified() -> list[NamedArch]:
    ...
    A standalone Researcher cannot be the final answerer (Researcher's
    responsibility is to gather, not to solve), so we always pair it with
    a Verifier that re-checks the drafted answer.
    """
    return [
        NamedArch(
...
def fam_moa_mini() -> list[NamedArch]:
    ...
    The "with Refiner" variant (2 Solvers + Refiner aggregator) lives in
    `fam_moa_fanin` (n=2 case). Keeping only the "no aggregator" variant
    here so the two families stay structurally distinct under family-
    stratified sampling.
    """
    return [
        NamedArch(
...
CANONICAL_FAMILIES = [
    # Singles (2)
    fam_single_solver, fam_researcher_verified,
    # 2-agent (5)
...
def core_library() -> list[NamedArch]:
    """Alias for `canonical_library()` — Tier 1 archetypes only.

    Used by callers that want the cleanly-designed archetypes without
    imperfect or random entries (e.g. for inspection in scripts).
    """
    return canonical_library()


def full_library(seed: int = 42) -> list[NamedArch]:
    """Tier 1 + Tier 2 + Tier 3 = full SFT target pool (93 entries: 68/15/10)."""
